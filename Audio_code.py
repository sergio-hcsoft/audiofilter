...
# Noisy environmet conversation corpus:
...
# Filter code
@jit(nopython=True)
def relativize_spectrogram(s, time_size=10, freq_size=5, redu = 0.5, pwr = 0.5, std_dev = 2.0):
    # Values in f range from -4 to 0 dB, I will exp it to (0, 1) for clarity
    #f = np.exp(s) * redu
    f = np.exp(s - redu) 
    for time in range(f.shape[0]):
        for freq in range(f.shape[1]):
            # We are filtering point (time, freq), lets look the cone around it
            tot = 0.0
            avg = 0.0
            min_t = min(f.shape[0], time + 1)
            # min_t = min(f.shape[0], time - time_size)
            max_t = min(f.shape[0], time + time_size)
            for t2 in range(min_t, max_t):
                for f2 in range(max(0, freq - freq_size), min(f.shape[1], freq + freq_size)):
                    weight = 1 + (time_size - abs(time - t2)) + (freq_size - abs(freq - f2))
                    weight = weight / (1 + time_size + freq_size)
                    weight = weight **2
                    tot = tot + weight    
                    avg = avg + f[t2, f2] * weight
            avg = avg / max(1.0, tot)
            #std = 0
            #for t2 in range(min(f.shape[0], time + 1), min(f.shape[0], time + time_size)):
            #    for f2 in range(max(0, freq - freq_size), min(f.shape[1], freq + freq_size)):
            #        weight = 1 + (time_size - abs(time - t2)) + (freq_size - abs(freq - f2))
            #        std = std + ((f[t2, f2] - avg) * weight)**2
            std = max(1.0, std_dev / max(1.0, tot))
            norm = ((f[time, freq] - avg) / std)
            
            mode = 1
                
            if mode == 1:
                coef = (1 + np.log(1 + norm)) if (norm > 0) else np.exp(norm)                        
                f[time, freq] = min(1.0, (f[time, freq] * coef**pwr))
            if mode == 2:
                coef = np.exp(-abs(norm))            
                if (f[time, freq] < avg):
                    f[time, freq] = (f[time, freq] * coef**pwr)
                else:
                    f[time, freq] = (1.0 - (1.0 - f[time, freq]) * coef**pwr)
            # np.clip raises errors so
            # f[time, freq] = max(-4, f[time, freq])
    f = np.log(f)
    #f = np.clip(f, -4, 0)
    return f


# ------------ Code form the outside world -----------------------
